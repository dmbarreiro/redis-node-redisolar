...
* Access to a local or remote installation of [Redis](https://redis.io/download) version 5 or newer with RedisTimeSeries module loaded

## Loading RedisTimeSeries module on local Redis

To load `RedistimeSeries` module on your local Redis database start cloning the module's repository:
...
The library will be in `bin/redistimeseries.so` ready to use.
Add the line

```
loadmodule path/RedisTimeSeries/bin/redistimeseries.so
```

to your Redis configuration file (`redis.conf`).
Start your local Redis database using the modified configuration file
