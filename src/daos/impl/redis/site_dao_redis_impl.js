...
const findAll = async () => {
  ...
  const siteKeys = await client.keysAsync('*');

  const promisedHashes = siteKeys.map(async (key) => {
    const ismember = await client.sismemberAsync(keyGenerator.getSiteIDsKey(), key);
    if (ismember === 1) {
      const siteHash = await client.hgetallAsync(key);
      return remap(siteHash);
    }
    return undefined;
  });
  const resolvedHashes = await Promise.all(promisedHashes);
  const siteHashes = resolvedHashes.filter((siteHash) => {
    return siteHash !== undefined;
  });
